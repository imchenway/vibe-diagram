...
`skills/vibe-diagram/` is the only editable source for skill behavior. That directory contains the bootstrap `SKILL.md`, 12 references, 60 HTML templates, a standard-library updater, and a standard-library linter. It has zero third-party runtime dependencies.
...
The updater downloads to a sibling staging directory, validates the release manifest and tree digest, moves the installed tree into a transient activation slot, and activates the verified candidate. It restores the installed tree if candidate promotion fails and deletes the transient old tree before reporting success. It retains no updater backup and provides no `--rollback` action; reinstall a pinned immutable tag when an older release is required. The first transition from an older backup-producing updater may briefly leave its legacy backup because that transition still runs the old code. On the next gate, the target updater removes only legacy-named trees whose `VERSION`, manifest, and complete tree digest all verify; manual or modified backups are preserved.
...
`promote-stable` requires persisted `TAG_VERIFIED` evidence and its own `--confirm-stable-promotion` authorization. It re-reads main, tag, Release, the current asynchronous workflow state, immutable tag ZIP, stable ancestry, and the stable manifest before writing. Workflow completion is not a promotion prerequisite. It accepts only a normal non-force fast-forward of the verified release commit. After the push it reads the stable commit, raw manifest, and immutable archive once. A synchronized response records `stable_validation: passed`; temporary raw/CDN lag records asynchronous pending evidence without waiting or rolling `stable` back.

`verify-runtime --mode isolated` installs the previous immutable tag under a temporary directory, upgrades through the published stable manifest, runs the target updater's current-version gate to remove any verified one-time legacy backup, proves that it retains neither a backup nor a staging directory, verifies offline fail-open behavior, performs a fresh archive installation, and removes both isolated installations. It does not touch the installed Skill and does not prove Codex client discovery. A passing isolated run is retained as prerequisite evidence while the release remains `STABLE_PROMOTED`.

`verify-runtime --mode installed-client` requires matching isolated evidence, an installed previous stable version, a new absolute artifact path, and the separate `--confirm-installed-skill-mutation` flag. It stages the immutable previous archive outside discovery, performs one real installed upgrade, runs the target updater's current-version gate to clean any verified one-time legacy backup, invokes `$vibe-diagram` in a fresh ephemeral `codex exec` process, validates the delivered HTML with the upgraded bundled linter, then temporarily isolates and restores the Skill while a second fresh process confirms non-discovery. Both modes must pass before the state becomes CLI-lane-scoped `RUNTIME_VERIFIED`. A post-mutation failure restores the staged immutable previous version in-process and records `PROMOTED_RUNTIME_FAILED`; no updater backup is retained, and the verifier never rewrites the released tag or moves `stable` backwards.
