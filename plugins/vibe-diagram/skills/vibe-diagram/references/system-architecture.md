...
- The 0.1.10 shared foundation is straight-first: adjacent architecture relationships have zero bends. A necessary orthogonal route is allowed only for crossing a boundary or avoiding a real obstacle, while empty nodes, content overflow, and semantically empty whitespace remain forbidden.
...
- Keep persistent 75%, 90%, 100%, and Auto controls on these two overview templates at measurable desktop widths. Their visibility is a template capability, not an overflow accident.
