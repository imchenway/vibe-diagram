...
| static-valid | The complete unit-test suite, a package-static-valid build, recomputable `docs/static-validation.json`, and the second complete unit-test run all passed. It does not mean that a client was installed or invoked. |
...
| public source set | The tracked publication content comprising product source, tests, package and CI definitions, stable documentation and decisions, plus generated Codex marketplace artifacts; transient task records are not part of it. |
...
| compatibility ledger | `docs/compatibility.md`, where static package state and real runtime state are recorded separately. |
