...
- 移除比较表和页面原型类型及其专用 HTML 生成入口，只保留五种基础图法；数据流仍是架构关系图的画法。
- 保留图形修改前后的差异对比，生成和交付检查拒绝已移除的类型。
- 从 0.2.2 升级本版需要使用新包升级器指定原安装目录，或完整重装；旧升级器仍要求已移除的 HTML 入口。

## [0.2.2] - 2026-09-09

- 将图形生成接入固定的 Archify 编译器和完整查看器，保留原有画布、网格、字体与颜色；实体关系 SVG 使用同一查看器，比较表和页面原型保留轻量 HTML。
- 同步升级包的原生运行文件清单，统一所有图类及作者 SVG 的文档标题，修复英文生成失败。
- 状态图只显示实际使用的分组，避免上游默认的空区域暗示不存在的业务过程。
- 从 0.2.1 迁移需要使用 0.2.2 升级器指定原安装目录，或完整重装；旧升级器会因已移除的排版文件拒绝新包。生成需要 Python 3.10+ 和 Node.js 18+，不新增安装依赖。
...
- 重构全局图表外壳：缩放组件统一进入标题控制区，每张画布在自身左上角显示归属明确的本地图例，窄屏时控件换行到标题下方。
- 代码审查页面采用左侧纵向意见导航和右侧三段阅读流，两张图共享常驻缩放，页面承担唯一纵向滚动，真实问题场景不参与缩放。
- 加强脚手架、严格校验、模板路由、确定性构建和 Codex 发布投影，失败关闭未知审查关系、现状与修复拓扑漂移、旧外壳结构和无归属图例。
