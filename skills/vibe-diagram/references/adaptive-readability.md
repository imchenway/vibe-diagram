...
Every graph-level title uses one `data-diagram-view-title="1"` heading with exactly one `data-diagram-view-type`, one `data-diagram-view-separator` that renders the full-width `｜`, and exactly one `data-diagram-view-subject`, in that order. A standalone artifact's page `h1` is also its only graph-level title and must use this structure. A multi-diagram package may keep an independent page title, but every subgraph still uses the structure. This global rule does not apply to table captions or detail headings.

### Local reading guide and title controls

- A local reading guide is optional at the global-shell level and required only when the selected family or template declares it. When present, it has one owning canvas, sits visibly inside that diagram surface at the top-left on the same continuous 24-pixel grid as the diagram body, has unique semantic groups, no ungridded separator strip, no separate `Reading guide` heading or card, and no page-level how-to-read prose. Group titles use a stronger label treatment than their items. Every relation item uses artifact-authored wording and binds to the actual relations it summarizes; the shell derives its swatch color and line style from those rendered relations. A family-specific contract may require exact groups and labels; for example, code review requires relationship types plus observed, checked, and not-yet-verified evidence states.
...
Fit width may choose a CSS scale from 75% through 100% only after measuring the unscaled authored stage. Every adaptive canvas declares `data-diagram-controls-mode="persistent"`; sequence canvases expose the same always-available percentage component through their toolbar. Manual percentages remain usable even while the stage already fits. If 75% cannot fit, keep semantic content unchanged and use scrolling. Re-test fit, selected scale, and persistent control visibility after the canvas or stage resizes. Print, no-JavaScript, reduced-motion, and runtime-error paths must remain readable without enhancement.

Use `data-diagram-title-region` and `data-diagram-title-copy` for the title and conclusion. Put controls only in `data-artifact-shell-controls` inside that title region. The shared shell CSS owns responsive alignment; the adaptive or sequence runtime owns fit and pressed-state updates but must not hide a measurable control set.
